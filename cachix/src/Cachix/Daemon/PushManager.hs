...
newPushManagerEnv :: (MonadIO m) => PushOptions -> NarinfoQuery.NarinfoQueryOptions -> PushParams PushManager () -> OnPushEvent -> Logger -> Tracer -> m PushManagerEnv
newPushManagerEnv pushOptions batchOptions pmPushParams onPushEvent pmLogger pmTracer = liftIO $ do
...
startBatchProcessor :: (MonadUnliftIO m, MonadReader env m, Tracing.HasTracer env, Katip.KatipContext m) => PushManagerEnv -> m ()
startBatchProcessor env@PushManagerEnv {pmNarinfoQueryManager} = do
  ...
  where
    ...
    processBatchedNarinfo storePaths = do
      ...
      Tracing.withDaemonSpan "cachix.daemon.narinfo.query_bulk" Internal $ \otelSpan -> do
...
resolvePushJob pushId closure = do
  Tracing.withDaemonSpan "cachix.daemon.push.resolve_job" Internal $ \otelSpan -> do
    let (statsText, totalCount, queuedCount, skippedCount) = closureStats closure
    addAttribute otelSpan "cachix.daemon.push.id" pushId
    addAttribute otelSpan "cachix.daemon.push.total_paths" totalCount
    addAttribute otelSpan "cachix.daemon.push.missing_paths" queuedCount
    addAttribute otelSpan "cachix.daemon.push.skipped_paths" skippedCount
