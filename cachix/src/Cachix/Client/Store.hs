...
import           Cachix.Client.Store.Context    ( context, Ref, NixStore, ValidPathInfo )
...
openStore :: IO Store
openStore = coerce [C.throwBlock| refStore* {
...
finalizeRefValidPathInfo = unsafePerformIO [C.exp|
...
finalizePathSet = unsafePerformIO [C.exp|
  void (*)(PathSet *) {
    [](PathSet *v){
      delete v;
    }
