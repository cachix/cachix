...
import Cachix.DebugInfod.Types (DebugInfodEnv (..), envToken)
...
fetchDebugInfoRedirect :: DebugInfodEnv -> Text -> IO (Maybe DebugInfo.DebugInfoRedirect)
fetchDebugInfoRedirect env buildId = do
  ...
  where
    tryFetch :: Text -> IO (Maybe DebugInfo.DebugInfoRedirect)
    tryFetch bid = do
      result <-
        retryHttp $
          (`runClientM` envClientEnv env) $
            API.getDebugInfo cachixClient (envToken env) (envCacheName env) bid
      pure $ case result of
        Left _ -> Nothing
        Right redirect -> Just redirect
