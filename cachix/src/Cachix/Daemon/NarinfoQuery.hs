...
submitRequest ::
  (MonadUnliftIO m, MonadReader env m, Tracing.HasTracer env, ToAttribute requestId) =>
...
start ::
  (MonadUnliftIO m, MonadReader env m, Tracing.HasTracer env, Katip.KatipContext m) =>
  NarinfoQueryManager requestId ->
...
processReadyBatch manager@NarinfoQueryManager {nqmCallback} processBatch ReadyBatch {rbRequests, rbAllPaths, rbBatchStartTime} =
  Tracing.withDaemonSpan "cachix.daemon.narinfo.process_batch" Internal $ \otelSpan -> do
    ...
    case rbBatchStartTime of
      ...
      Just startTime -> do
        ...
        addAttribute otelSpan "cachix.daemon.narinfo.wait_time_ms" (waitTimeMs :: Double)
    ...
    (allPathsInClosure, missingPaths) <-
      if null rbAllPaths
        then return ([], [])
          ...
          addAttribute otelSpan "cachix.daemon.narinfo.processing_time_ms" (processingTimeMs :: Double)
          addAttribute otelSpan "cachix.daemon.narinfo.total_paths" closureSize
          addAttribute otelSpan "cachix.daemon.narinfo.missing_paths" missingCount
          addAttribute otelSpan "cachix.daemon.narinfo.hit_paths" hitCount
