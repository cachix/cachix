...
findFilesByName dir filename = do
  ...
  if not exists
    then pure []
    else walkDir dir "" filename 0

maxWalkDepth :: Int
maxWalkDepth = 64

walkDir :: FilePath -> FilePath -> FilePath -> Int -> IO [FilePath]
walkDir root relative filename depth
  ...
  | otherwise = do
      ...
      results <- for entries $ \entry -> do
        ...
        if isDir
          then walkDir root relPath filename (depth + 1)
          else
