...
- client: `cachix authtoken --secretspec` and `cachix generate-keypair --secretspec` store the auth token or signing key in your default secretspec provider under the `cachix` project namespace, instead of the configuration file. Requires the `secretspec` CLI.
