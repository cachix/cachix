...
new daemonEnv nixStore daemonOptions daemonLogHandle daemonPushOptions daemonCacheName = do
  ...
  daemonTracer <- Tracing.getDaemonTracer
  daemonPushManager <- PushManager.newPushManagerEnv daemonPushOptions (daemonNarinfoQueryOptions daemonOptions) pushParams onPushEvent daemonLogger daemonTracer
...
run daemon = fmap join <$> runDaemon daemon $
  Tracing.withDaemonSpan "cachix.daemon.run" Internal $ \_ -> do
    ...
    eventLoopRes <- EventLoop.run daemonEventLoop $ \event ->
      Tracing.withDaemonSpan "cachix.daemon.event_loop.process" Internal $ \otelSpan -> do
        addAttribute otelSpan "cachix.daemon.event.type" (daemonEventName event)
        case event of
          ...
          ReceivedMessage socketId clientMsg ->
            Tracing.withDaemonSpan "cachix.daemon.client_message" Server $ \clientSpan -> do
              addAttribute clientSpan "cachix.daemon.socket_id" (UUID.toText socketId)
              addAttribute clientSpan "cachix.daemon.client.message" (clientMessageName clientMsg)
              case clientMsg of
                ClientPushRequest pushRequest -> do
                  ...
                  addAttribute clientSpan "cachix.daemon.push.id" pushId
                ...
                ClientDiagnosticsRequest ->
                  Tracing.withDaemonSpan "cachix.daemon.diagnostics" Internal $ \_ -> do
...
shutdownGracefully = do
  Tracing.withDaemonSpan "cachix.daemon.shutdown" Internal $ \_ -> do
    DaemonEnv {..} <- ask

    Tracing.withDaemonSpan "cachix.daemon.shutdown.stop_batch_processor" Internal $ \_ ->
      liftIO $ PushManager.stopBatchProcessor daemonPushManager

    Tracing.withDaemonSpan "cachix.daemon.shutdown.stop_push_manager" Internal $ \_ ->
      shutdownPushManager daemonPushManager

    Tracing.withDaemonSpan "cachix.daemon.shutdown.stop_workers" Internal $ \_ ->
      withTakeMVar daemonWorkerThreads Worker.stopWorkers

    Tracing.withDaemonSpan "cachix.daemon.shutdown.stop_subscriptions" Internal $ \_ ->
    ...
    Tracing.withDaemonSpan "cachix.daemon.shutdown.close_connections" Internal $ \_ ->
