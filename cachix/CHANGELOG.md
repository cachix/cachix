...
- client: `cachix authtoken` and `cachix generate-keypair` store credentials via secretspec by default when it is set up (the `secretspec` CLI on PATH and a default provider configured), under the `cachix` project namespace, and move any credential out of the configuration file that would shadow it. Signing keys get a per cache slot (`CACHIX_SIGNING_KEY_<cache>`) through a secret reference. `--secretspec` forces secretspec, `--no-secretspec` keeps the configuration file, and a bare `cachix authtoken` prompts for the token with hidden input via secretspec.
